...
export default function NewArticlePage() {
  ...
  const [category, setCategory] = useState("Quran & Tajweed");
  const [author, setAuthor] = useState("Sheikh Ahmed Al-Azhari");
  const [authorRole, setAuthorRole] = useState("Senior Tajweed & Qira'at Scholar");
  ...
  // مساعد لإدراج التنسيقات عند المؤشر
  ...
  // رفع ملف عام (سواء غلاف أو داخل النص)
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMsg("Failed to upload cover: " + err.message);
    } finally {
      setUploadingCover(false);
    }
  };

  // معالجة رفع صورة داخل نص المقال
  const handleInlineImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadImageFile(file);
      // إدراج الصورة بصيغة Markdown مع إمكانية كتابة وصف
    ...
    try {
      const res = await fetch("/api/admin/articles", {
        ...
        body: JSON.stringify({
          ...
          cover_image: coverImage || "https://images.unsplash.com/photo-1609599006353-e629aaabfeae?auto=format&fit=crop&w=1200&q=80",
        }),
      });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mt-1">Write New Article</h1>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 5 Common Mistakes in Quran Recitation"
          ...
          {/* Excerpt */}
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Excerpt / Brief Hook</label>
            <textarea
              rows={2}
              required
              value={excerpt}
          ...
          {/* Cover Image Uploader (Local File or URL) */}
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Cover Image (Upload from PC or enter URL)</label>
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                >
                  ...
                  <span>{uploadingCover ? "Uploading Image..." : "Upload from Device"}</span>
                </button>
                <span className="text-xs text-slate-400 font-semibold">or paste direct link:</span>
              </div>
              ...
              <div className="flex gap-3 items-center">
                ...
                />
                {coverImage && (
                  <img src={coverImage} alt="Cover Preview" className="w-16 h-10 object-cover rounded-lg border border-slate-200" />
                )}
              </div>
            </div>
          </div>
          ...
          {/* Advanced Rich Text Toolbar with Colors & Inline Images */}
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-bold text-slate-700">Article Body Content</label>
              <span className="text-[11px] text-slate-400">Markdown & Media Supported</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 p-2 bg-slate-100 rounded-t-xl border border-b-0 border-slate-300">
              ...
              <button
                ...
                className="px-2.5 py-1 bg-white hover:bg-slate-200 border rounded text-xs italic text-slate-800"
              >
                <i>I</i>
              </button>
              ...
              <button
                type="button"
                onClick={() => insertFormatting('<span class="text-rose-700 font-semibold">', '</span>', 'Red text')}
                className="px-2 py-1 bg-rose-100 hover:bg-rose-200 text-rose-900 rounded text-xs font-bold"
                title="Red Text"
              >
                Rose
              </button>
          ...
          >
            {loading ? "Publishing Article..." : "Publish Article to Blog"}
          </button>
        </form>
      </div>
    </div>
